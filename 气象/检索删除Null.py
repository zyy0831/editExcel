...
def getLabel(labels,labelFin):
    ex1 = 'Unnamed'
    for label in labels:
        if (ex1 in label) or ('o' in label)or ('y' in label):
            result= label
        else:
...
def main(folderPath,pathout,newFileName):
    ...
    dfTemp = pd.read_excel(xlsx_names[-1])
    labels = list(dfTemp.columns.values)
    ...
    for sheet_name in sheet_names:
        ...
        for i in df:
            if df.loc[:, i].isnull().sum(axis=0) >= 15: 
...
def Deal(folderPathArr):
    for folderpath in folderPathArr:
        ...
        newFileName = folderpath[-6:] #截取倒数第六位到结尾
...
if __name__=="__main__":
    # path = r'c:\Users\zyy\Desktop\气象\城市_20190101-20191231-xlsx\outt'
    path = input("请输入待处理文件\n")
    pathout = r'c:\Users\zyy\Desktop\气象\城市_20190101-20191231-xlsx\finn'
    #创建文件夹
